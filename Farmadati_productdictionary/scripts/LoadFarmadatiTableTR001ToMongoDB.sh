#!/bin/sh
# 
# Welfinity Farmadati Upload Table TR001 to MongoDB
# Author : Cristiano Ressi di Cervia , Welfinity
# date : 30/08/2017
# version : 1.0
#
# The aim of this job is to upload Farmadati Table TR001 to MongoDB instance in the welfinity private cloud
# 
# Parameters
# ---------------
...
FILENAME="$NOW-LoadFarmadatiTableTR001ToMongoDB.log"

exec 3>&1 4>&2
trap 'exec 2>&4 1>&3' 0 1 2 3
exec 1>>/var/log/welfinity/script/farmadati/$FILENAME 2>&1

LOG_DATE=$(date +"%Y-%m-%d %H:%M:%S [LoadFarmadatiTableTR001ToMongoDB]")
...
if [ $# != 7 ]; then
	...
	echo "Example : ./LoadFarmadatiTableTR001ToMongodb.sh '217.182.194.167' 27017 Product_Dictionaries_Italy TR001  talendUser 'ba+Req6@agu6' '/data/Farmadati/logs/'"
	exit
fi
...
/opt/welfinity/talend/jobs/farmadati/Farmadati_TR001/Farmadati_TR001/Farmadati_TR001_run.sh --context_param database_url=$1 --context_param database_port=$2 --context_param database=$3 --context_param collection=$4 --context_param user=$5 --context_param password=$6 --context_param logDirectory=$7
