#!/bin/sh
# 
# Welfinity Farmadati Upload Table TS026 to MongoDB
# Author : Cristiano Ressi di Cervia , Welfinity
# date : 30/08/2017
# version : 1.0
#
# The aim of this job is to upload Farmadati Table TS026 to MongoDB instance in the welfinity private cloud
# 
# Parameters
# ---------------
...
# collection	    mongodb collection      - default value :	TS026
...
FILENAME="$NOW-LoadFarmadatiTableTS026ToMongoDB.log"

exec 3>&1 4>&2
trap 'exec 2>&4 1>&3' 0 1 2 3
exec 1>>/var/log/welfinity/script/farmadati/$FILENAME 2>&1

LOG_DATE=$(date +"%Y-%m-%d %H:%M:%S [LoadFarmadatiTableTS026ToMongoDB]")
...
if [ $# != 7 ]; then
	...
	echo "Example : ./LoadFarmadatiTableTS026ToMongodb.sh '217.182.194.167' 27017 Product_Dictionaries_Italy TS026  talendUser 'ba+Req6@agu6' "
	exit
fi
...
/opt/welfinity/talend/jobs/farmadati/Farmadati_TS026/Farmadati_TS026/Farmadati_TS026_run.sh  --context_param database_uri=$1 --context_param database_port=$2 --context_param database=$3 --context_param collection=$4 --context_param user=$5 --context_param password=$6 --context_param logDirectory=$7
