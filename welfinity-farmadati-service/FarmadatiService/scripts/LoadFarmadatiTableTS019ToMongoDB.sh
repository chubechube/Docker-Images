#!/bin/sh
# 
# Welfinity Farmadati Upload Table TS019 to MongoDB
# Author : Cristiano Ressi di Cervia , Welfinity
# date : 30/08/2017
# version : 1.0
#
# The aim of this job is to upload Farmadati Table TS019 to MongoDB instance in the welfinity private cloud
# 
# Parameters
# ---------------
...
# collection	    mongodb collection      - default value :	TS019
...
FILENAME="$NOW-LoadFarmadatiTableTS019ToMongoDB.log"

exec 3>&1 4>&2
trap 'exec 2>&4 1>&3' 0 1 2 3
exec 1>>/var/log/welfinity/script/farmadati/$FILENAME 2>&1

LOG_DATE=$(date +"%Y-%m-%d %H:%M:%S [LoadFarmadatiTableTS019ToMongoDB]")



# checking parameters before anything
if [ $# != 7 ]; then
	echo "Welfinity LoadFarmadatiTableTS019 Job"
	echo "----------------------------------"
	...
	echo "Example : ./LoadFarmadatiTableTS019ToMongodb.sh '217.182.194.167' 27017 Product_Dictionaries_Italy TS019  talendUser 'ba+Req6@agu6' "
	exit
fi
